...
describe('Tier 1: Earned and Total Rewards Correct ', async function() {
    ...
    it('[1-12 weeks]', async function () {
       ...
       // first 12 accounts[0-11] mint 1 each and set commitment 1-12
        for(let i = 0; i < 12; i++) {
            ...
            signatures.push(await createSignature([i+1], [1], this.accounts[0]));
        }
        ...
        for(let i = 0; i < 12; i++) {
            ...
            await this.staking.connect(this.accounts[i]).stake([i+1],[1], [i+1], signatures[i]);
            ...
            for(let j = 1; j <= i+1; j++) {
                ...
                let expected = rewardChart[0][1][j];
